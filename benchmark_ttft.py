...
def append_results(new_results, output_file="paper_model_results_ttft.json"):
...
def benchmark_inference_torch_compile(model, inputs):
...
def benchmark_inference_torch_xla(model, inputs):
    start = time.time()
    _ = model(**inputs)
    xm.mark_step()
    end = time.time()
...
# def benchmark_training_torch_xla(model, inputs):
#     model.train()
#     optimizer = torch.optim.SGD(model.parameters(), lr=1e-3)
#     loss_fn = torch.nn.MSELoss()
#     with torch.no_grad():
...
def get_dummy_inputs_tf(tokenizer):
    """Return a batch of size 1 as tf.Tensors."""
    dummy_text = "This is a dummy input for benchmarking."
...
def benchmark_inference_tf(model, inputs):
    # Timed runs
    start = time.time()
    _ = model(**inputs)
    end = time.time()
...
# def benchmark_training_tf(model, inputs):
...
def benchmark_inference_tf_xla(model, inputs):
...
def benchmark_inference_flax(model, inputs):
    ...
    for original_name, spec in tqdm(mapping.items(),
        ...
        try:
            try:
                ...
                # ─── JAX/Flax benchmark ───
                try:
                    # dynamically pick e.g. FlaxAlbertForMaskedLM, etc.
                ...
                try:
                    # dynamically pick e.g. TFAutoModelForSequenceClassification, TFAutoModelForMaskedLM, etc.
...
if __name__ == "__main__":
    json_path = "official_models.json"  # Your JSON file containing model definitions
